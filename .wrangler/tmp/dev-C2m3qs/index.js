...
// .wrangler/tmp/bundle-Ujjf16/checked-fetch.js
...
// src/types.ts
var WORKOUT_CATEGORIES = ["Upper Body", "Lower Body", "Cardio HIIT", "Whole Body"];
var MUSCLE_GROUPS = [
  "Chest",
  "Back",
  "Shoulders",
...
var EQUIPMENT_TYPES = ["BB", "DB", "KB", "Machine", "Bodyweight", "Trap Bar"];
var WORKOUT_TAGS = ["with cadence"];
var WEIGHT_METHODS = ["ES", "One", "Total"];
var PRESET_SETS = {
...
var WORKOUT_TYPES_PLAN = ["Single sets", "Superset", "Custom"];
var WEIGHT_INCREMENTS = {
  BB: 2.5,
  DB: 2,
  KB: 4,
  Machine: 5,
...
// src/db.ts
async function getExercises(db, filters = {}) {
  let query = `
    SELECT e.*,
...
__name(getExercises, "getExercises");
async function getExerciseById(db, id) {
  const result = await db.prepare("SELECT * FROM exercises WHERE id = ?").bind(id).first();
  return result;
}
__name(getExerciseById, "getExerciseById");
async function getExerciseByName(db, name) {
  const result = await db.prepare("SELECT * FROM exercises WHERE name = ?").bind(name).first();
  return result;
}
__name(getExerciseByName, "getExerciseByName");
async function searchExercises(db, query, limit = 20) {
  const result = await db.prepare("SELECT * FROM exercises WHERE name LIKE ? ORDER BY name LIMIT ?").bind(`%${query}%`, limit).all();
  return result.results;
}
__name(searchExercises, "searchExercises");
async function createExercise(db, data) {
...
__name(createExercise, "createExercise");
async function updateExercise(db, id, data) {
  const updates = [];
  const params = [];
  if (data.name !== void 0) {
    updates.push("name = ?");
  ...
  if (data.muscle_main !== void 0) {
    updates.push("muscle_main = ?");
    params.push(data.muscle_main || null);
  }
  if (data.muscle_additional !== void 0) {
  ...
  if (data.weight_min !== void 0) {
    updates.push("weight_min = ?");
    params.push(data.weight_min && data.weight_min > 0 ? data.weight_min : null);
  }
  if (data.weight_max !== void 0) {
    updates.push("weight_max = ?");
    params.push(data.weight_max && data.weight_max > 0 ? data.weight_max : null);
  }
  if (updates.length === 0) return false;
  params.push(id);
  const result = await db.prepare(`UPDATE exercises SET ${updates.join(", ")} WHERE id = ?`).bind(...params).run();
  return result.meta.changes > 0;
}
__name(updateExercise, "updateExercise");
async function deleteExercise(db, id) {
  const logsResult = await db.prepare("SELECT COUNT(*) as count FROM workout_logs WHERE exercise_id = ?").bind(id).first();
...
__name(deleteExercise, "deleteExercise");
async function getOrCreateExercise(db, name, category, subcategory, equipment_type) {
  const existing = await getExerciseByName(db, name);
  if (existing) return existing.id;
  const result = await db.prepare("INSERT INTO exercises (name, category, subcategory, equipment_type) VALUES (?, ?, ?, ?)").bind(name, category, subcategory || null, equipment_type || null).run();
  return result.meta.last_row_id;
}
__name(getOrCreateExercise, "getOrCreateExercise");
async function getUniqueMuscles(db) {
  const mainResult = await db.prepare("SELECT DISTINCT muscle_main FROM exercises WHERE muscle_main IS NOT NULL ORDER BY muscle_main").all();
  const additionalResult = await db.prepare("SELECT DISTINCT muscle_additional FROM exercises WHERE muscle_additional IS NOT NULL").all();
  const additionalSet = /* @__PURE__ */ new Set();
...
__name(getUniqueMuscles, "getUniqueMuscles");
async function getWorkoutHistory(db, filters = {}) {
  let query = `
...
__name(getWorkoutHistory, "getWorkoutHistory");
async function getWorkoutLogById(db, id) {
  const result = await db.prepare(
...
__name(getWorkoutLogById, "getWorkoutLogById");
async function createWorkoutLog(db, data) {
  ...
  const result = await db.prepare(
    `INSERT INTO workout_logs (exercise_id, weight_kg, reps, sets, workout_date, notes, tags)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).bind(exerciseId, data.weight_kg, data.reps, data.sets, data.workout_date, data.notes || "", tagsStr).run();
  return result.meta.last_row_id;
}
__name(createWorkoutLog, "createWorkoutLog");
async function updateWorkoutLog(db, id, data) {
  const updates = [];
  const params = [];
  if (data.weight_kg !== void 0) {
  ...
  if (updates.length === 0) return false;
  params.push(id);
  const result = await db.prepare(`UPDATE workout_logs SET ${updates.join(", ")} WHERE id = ?`).bind(...params).run();
  return result.meta.changes > 0;
}
__name(updateWorkoutLog, "updateWorkoutLog");
async function deleteWorkoutLog(db, id) {
  const result = await db.prepare("DELETE FROM workout_logs WHERE id = ?").bind(id).run();
  return result.meta.changes > 0;
}
__name(deleteWorkoutLog, "deleteWorkoutLog");
async function getExerciseProgression(db, exerciseId) {
  const result = await db.prepare(
...
__name(getExerciseProgression, "getExerciseProgression");
async function getLatestWorkoutForExercise(db, exerciseId) {
  const result = await db.prepare(
...
__name(getLatestWorkoutForExercise, "getLatestWorkoutForExercise");
async function getAllWorkoutLogsForExport(db) {
  const result = await db.prepare(
...
__name(getAllWorkoutLogsForExport, "getAllWorkoutLogsForExport");
async function getWorkoutPlan(db, planDate) {
  const result = await db.prepare("SELECT * FROM workout_plans WHERE plan_date = ?").bind(planDate).first();
  return result;
}
__name(getWorkoutPlan, "getWorkoutPlan");
async function getRecentWorkoutPlans(db, limit = 10) {
  const result = await db.prepare("SELECT * FROM workout_plans ORDER BY plan_date DESC LIMIT ?").bind(limit).all();
  return result.results;
}
__name(getRecentWorkoutPlans, "getRecentWorkoutPlans");
async function saveWorkoutPlan(db, data) {
  const existing = await getWorkoutPlan(db, data.plan_date);
  const noShowValue = data.no_show ? 1 : 0;
  if (existing) {
    await db.prepare("UPDATE workout_plans SET theme = ?, workout_type = ?, branch = ?, description = ?, no_show = ? WHERE plan_date = ?").bind(data.theme, data.workout_type || null, data.branch || null, data.description || null, noShowValue, data.plan_date).run();
    return existing.id;
  } else {
    const result = await db.prepare("INSERT INTO workout_plans (plan_date, theme, workout_type, branch, description, no_show) VALUES (?, ?, ?, ?, ?, ?)").bind(data.plan_date, data.theme, data.workout_type || null, data.branch || null, data.description || null, noShowValue).run();
    return result.meta.last_row_id;
  }
}
__name(saveWorkoutPlan, "saveWorkoutPlan");
async function deleteWorkoutPlan(db, planDate) {
  const result = await db.prepare("DELETE FROM workout_plans WHERE plan_date = ?").bind(planDate).run();
  return result.meta.changes > 0;
}
__name(deleteWorkoutPlan, "deleteWorkoutPlan");
async function getWorkoutPlanStations(db, planId) {
  const result = await db.prepare("SELECT * FROM workout_plan_stations WHERE plan_id = ? ORDER BY station_number").bind(planId).all();
  return result.results;
}
__name(getWorkoutPlanStations, "getWorkoutPlanStations");
async function saveWorkoutPlanStations(db, planId, stations) {
...
__name(saveWorkoutPlanStations, "saveWorkoutPlanStations");

// src/routes/exercises.ts
var exercises = new Hono2();
...
workouts.get("/", async (c) => {
  ...
  const result = await getWorkoutHistory(c.env.DB, {
    exercise_id: exercise_id ? parseInt(exercise_id) : void 0,
    category,
    workout_date,
...
var app = new Hono2();
app.use("/api/*", cors());
app.route("/api/exercises", exercises_default);
app.route("/api/workouts", workouts_default);
...
app.post("/api/upload-image", async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare("UPDATE exercises SET image_url = ? WHERE id = ?").bind(data, exercise_id).run();
...
// .wrangler/tmp/bundle-Ujjf16/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Ujjf16/middleware-loader.entry.ts
